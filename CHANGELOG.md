...
- Outbound (egress) fault injection in the sidebar, with env var + remote base URL inputs and caching

### Changed

- Improved inbound/outbound fault injection narrative and flow guidance in the sidebar

### Fixed

- Prevent duplicated commands in integrated terminals by disabling VS Code shell integration for fault terminals
...
### Changed
- The fault injection panel now runs `fault run` with TCP proxy mapping (`--proxy "9090=..."`) and opens a split terminal with a curl command targeting `http://127.0.0.1:9090/...`.
- Removed scenario generation/report flow for now.

### Added
- New fault templates: jitter, bandwidth throttling, mobile-edge emulation, and scheduled brownout/outage windows.
- A short intent/description for each fault template.
...
### Changed
- Lowered minimum supported VS Code version to 1.102.
